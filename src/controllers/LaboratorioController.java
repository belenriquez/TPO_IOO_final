...
public class LaboratorioController {
	...
	private void recuperarSucursalesGuardadas() {
		Gson gson = new Gson();
		java.lang.reflect.Type listType = new TypeToken<ArrayList<Sucursal>>() {
		}.getType();
	...
	public Sucursal getSucursalByNroSuc(int nroSuc) {
		Sucursal sucursal = null;
		int i = 0;
		while (sucursal == null && i != sucursales.size()) {
			if (sucursales.get(i).getNumeroSucursal() == nroSuc) {
				sucursal = sucursales.get(i);
			}
			i++;
		}
	...
	private void recuperarPeticionesGuardadas() {
		Gson gson = new Gson();
		java.lang.reflect.Type listType = new TypeToken<ArrayList<Peticion>>() {
		}.getType();
	...
	public boolean altaPeticion(Peticion peticion) {
		...
		for(Practica p : peticion.getPracticasAsociadas()) {
			PracticaPeticion nuevo = new PracticaPeticion(p.getCodigoPractica(), peticion.getIdPeticion());
			nuevo.setDniPaciente(peticion.getPaciente().getDni());
			if(practicasPeticiones == null) {
				practicasPeticiones = new ArrayList<PracticaPeticion>();
			}
			practicasPeticiones.add(nuevo);
		}
		
		guardarPracticasPeticiones();
		guardarPeticiones();
		return true;
	}
	...
	private void recuperarPracticasPeticionesGuardadas() {
		Gson gson = new Gson();
		java.lang.reflect.Type listType = new TypeToken<ArrayList<PracticaPeticion>>() {
		}.getType();
		try (FileReader reader = new FileReader("src/json/practicapeticiones.json")) {
	...
	private void guardarPracticasPeticiones() {
		try (Writer writer = new FileWriter("src/json/practicapeticiones.json")) {
			Gson gson = new GsonBuilder().create();
	...
	public int generateIdSucursal() {
		Sucursal aux = this.getSucursal(this.sucursales.size());
		while(aux != null) {
			aux = this.getSucursal(this.sucursales.size() + 1);
		}
		return this.sucursales.size() + 1;
	}

	public int generateIdPeticion() {
		Peticion aux = this.getPeticion(this.peticiones.size());
		while(aux != null) {
			aux = this.getPeticion(this.peticiones.size() + 1);
		}

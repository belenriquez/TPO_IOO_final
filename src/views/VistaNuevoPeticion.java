...
public class VistaNuevoPeticion extends JFrame {
		...
		btnGuardar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (Object ppo : ((DefaultListModel) list_practicasPedidas.getModel()).toArray() ){
					...
					pp.setResultado(new Resultado());
					pedidas.add( pp);
				}

				Peticion nuevaPeticion = new Peticion(-1,Integer.parseInt(textField_dniPaciente.getText()),textField_obraSocial.getText(),idSucursal,pedidas);
	...
	public void editarPeticion(String cod) {
		...
		List<PracticaPeticion> practicasPedidas = LaboratorioController.getInstancia().getPracticasPedidasByPeticion(Integer.parseInt(cod));
